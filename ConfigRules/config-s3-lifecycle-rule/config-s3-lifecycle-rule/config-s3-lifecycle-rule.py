...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    tags = client.get_bucket_tagging(s3_bucket_name)
    for tag in tags['TagSet']:
        #If the bucket is tagged a Key : Value of DataCompliance : <Paramter Tag Input> evaluate compliance
        if tag['Key'] is 'DataCompliance' and tag['Value'] is valid_rule_parameters['tag']:
            #Get the bucket lifecycle
            bucket_lifecycle = client.get_bucket_lifecycle_configuration(s3_bucket_name)
            ...
            for rule in rules:
                #If the rule has either a Tag or And key, check to see if it is the rule for <Paramter Tag Input> Compliance.
                #If the Tag matched, get the expiration days for the rule and and set it to placeholder
                if 'Tag' in rule['Filter']: 
                    tag = rule['Filter']['Tag']
                    if tag['Key'] is valid_rule_parameters['tag']:
            ...
            if days_to_expire >= int(valid_rule_parameters['daysToRetain']):
                ...
                return build_evaluation_from_config_item(configuration_item, 'NON_COMPLIANT', 'The lifecycle days ({}) is less than the expected ({}).'.format(days_to_expire, valid_rule_parameters['s3Lifecycle']['daysToRetain']))
