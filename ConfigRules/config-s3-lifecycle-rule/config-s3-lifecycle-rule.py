...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    print(configuration_item["resourceName"])

    #Get S3 Bucket Name
    s3_bucket_name = configuration_item["resourceName"]
    #Initiate AWS S3 Client
    client = get_client('s3', event)
    #Get the Tags for the S3 Bucket
    try:
        tags = client.get_bucket_tagging(Bucket=s3_bucket_name)
    except Exception:
        print('Bucket does not have tags')
        return build_evaluation_from_config_item(configuration_item, 'COMPLIANT', 'Bucket has not tags')
    #If there are tags, see if it has a compliance requirement
    for tag in tags['TagSet']:
        print(tag)
        #If the bucket is tagged a Key : Value of DataCompliance : <Paramter Tag Input> evaluate compliance
        if tag['Key'] == 'DataCompliance' and tag['Value'] == valid_rule_parameters['tag']:
            print(tag)
            #Get the bucket lifecycle
            try:
                bucket_lifecycle = client.get_bucket_lifecycle_configuration(Bucket=s3_bucket_name)
            except Exception as e:
                print('No Lifecycle Policy applied to bucket')
                return build_evaluation_from_config_item(configuration_item, 'NON_COMPLIANT', 'No Lifecycle Policy applied to bucket.')
            #set placeholder for lifecycle days to expire
            days_to_expire = 0
            #get the rules from the lifecycle and loop through each one
            rules = bucket_lifecycle['Rules']
            #Loop through the rules and see if one has an expiration of at least 7 years
            for rule in rules:
                print(rule)
                if rule['Expiration']['Days'] > days_to_expire:
                    days_to_expire = rule['Expiration']['Days']
                    print('setting days_to_expire to rule expiration')

            #If the days to expire placeholder are more than the retention policy days in paramters, return COMPLIANT, otherwise NONCOMPLIANT
            print(days_to_expire)
            print(valid_rule_parameters['daysToRetain'])
            if days_to_expire >= int(valid_rule_parameters['daysToRetain']):
                return build_evaluation_from_config_item(configuration_item, 'COMPLIANT')
            else:
                return build_evaluation_from_config_item(configuration_item, 'NON_COMPLIANT', 'The lifecycle days ({}) is less than the expected ({}).'.format(days_to_expire, valid_rule_parameters['daysToRetain']))
    #If the Bucket is not tagged with <Parameter Tag Input> then return COMPLIANT, as bucket does not need to have the required retention policy
    return build_evaluation_from_config_item(configuration_item, 'COMPLIANT', 'Bucket Not Tagged with FINRA Compliance')
